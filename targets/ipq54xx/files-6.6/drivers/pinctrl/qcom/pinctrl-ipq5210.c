...
static const struct msm_pingroup ipq5210_groups[] = {
	[0] = PINGROUP(0, sdc_data, qspi_data, pwm_out32, _, _, _, _, _, _, _, _),
	[1] = PINGROUP(1, sdc_data, qspi_data, pwm_out22, _, _, _, _, _, _, _, _),
	[2] = PINGROUP(2, sdc_data, qspi_data, pwm_out12, _, _, _, _, _, _, _, _),
	[3] = PINGROUP(3, sdc_data, qspi_data, pwm_out02, _, _, _, _, _, _, _, _),
	[4] = PINGROUP(4, sdc_cmd, qspi_cs_n, _, _, _, _, _, _, _, _, _),
	[5] = PINGROUP(5, sdc_clk, qspi_clk, _, _, _, _, _, _, _, _, _),
	[6] = PINGROUP(6, qup_se0_l2, led02, pwm_out31, _, cri_trng0, qdss_tracedata_a, _, _, _, _, _),
	[7] = PINGROUP(7, qup_se0_l3, led12, pwm_out21, _, cri_trng1, qdss_tracedata_a, _, _, _, _, _),
	[8] = PINGROUP(8, qup_se0_l0, pwm_out11, audio_pri2, audio_pri2, _, cri_trng2, qdss_tracedata_a, _, _, _, _),
	[9] = PINGROUP(9, qup_se0_l1, led22, pwm_out01, _, cri_trng3, qdss_tracedata_a, _, _, _, _, _),
	...
	[17] = PINGROUP(17, pon_tx_fault, audio_sec0, audio_sec0, _, _, _, _, _, _, _, _),
	[18] = PINGROUP(18, pps, pll_test, _, _, _, _, _, _, _, _, _),
	[19] = PINGROUP(19, mux_tod_out, audio_pri1, audio_pri1, _, _, _, _, _, _, _, _),
	[20] = PINGROUP(20, qup_se2_l10, mdc_slv1, tsens_max, qdss_tracedata_a, _, _, _, _, _, _, _),
	[21] = PINGROUP(21, qup_se2_l00, mdio_slv1, qdss_tracedata_a, _, _, _, _, _, _, _, _),
	[22] = PINGROUP(22, core_voltage_0, qup_se3_l1, pwm_out03, _, _, _, _, _, _, _, _),
	[23] = PINGROUP(23, Led00, qup_se3_l0, dbg_out_clk, qdss_traceclk_a, _, _, _, _, _, _, _),
	[24] = PINGROUP(24, _, _, _, _, _, _, _, _, _, _, _),
	[25] = PINGROUP(25, _, _, _, _, _, _, _, _, _, _, _),
	[26] = PINGROUP(26, mdc_mst, led21, _, qdss_tracectl_a, _, _, _, _, _, _, _),
	[27] = PINGROUP(27, mdio_mst, led11, _, _, _, _, _, _, _, _, _),
	[28] = PINGROUP(28, pcie1_clk_req_n, qup_se1_l1, _, _, qdss_cti_trig_out_a0, _, _, _, _, _, _),
	[29] = PINGROUP(29, _, _, _, _, _, _, _, _, _, _, _),
	[30] = PINGROUP(30, pcie1_wake, qup_se1_l0, _, _, qdss_cti_trig_in_a0, _, _, _, _, _, _),
	[31] = PINGROUP(31, pcie0_clk_req_n, mdc_slv0, _, qdss_cti_trig_out_a1, _, _, _, _, _, _, _),
	[32] = PINGROUP(32, _, _, _, _, _, _, _, _, _, _, _),
	[33] = PINGROUP(33, pcie0_wake, mdio_slv0, qdss_cti_trig_in_a1, _, _, _, _, _, _, _, _),
	[34] = PINGROUP(34, audio_pri_d0, atest_char_status0, qdss_cti_trig_in_b0, _, _, _, _, _, _, _, _),
	[35] = PINGROUP(35, audio_pri_d1, rx_los21, atest_char_status1, qdss_cti_trig_out_b1, _, _, _, _, _, _, _),
	[36] = PINGROUP(36, audio_pri_fsync, _, rx_los11, atest_char_status2, _, _, _, _, _, _, _),
	[37] = PINGROUP(37, audio_pri_pclk, rx_los01, atest_char_status3, _, qdss_cti_trig_in_b1, _, _, _, _, _, _),
	[38] = PINGROUP(38, qup_se1_l3, led20, gcc_plltest_bypassnl, qdss_tracedata_a, _, _, _, _, _, _, _),
	[39] = PINGROUP(39, qup_se1_l2, led10, led01, gcc_tlmm, qdss_tracedata_a, _, _, _, _, _, _),
	[40] = PINGROUP(40, qup_se4_l2, rx_los20, audio_sec_fsync, gcc_plltest_resetn, qdss_tracedata_a, _, _, _, _, _, _),
	[41] = PINGROUP(41, qup_se4_l3, rx_los10, audio_sec_pclk, qdss_tracedata_a, _, _, _, _, _, _, _),
	[42] = PINGROUP(42, qup_se4_l0, rx_los00, audio_sec_d1, atest_tic_en, _, _, _, _, _, _, _),
	[43] = PINGROUP(43, qup_se4_l1, audio_sec_d0, _, _, _, _, _, _, _, _, _),
...
static const struct of_device_id ipq5210_tlmm_of_match[] = {
	{ .compatible = "qcom,ipq5210-tlmm", .data = &ipq5210_tlmm },
	{},
};
