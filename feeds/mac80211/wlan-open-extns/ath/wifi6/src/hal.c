...
const struct ath12k_hw_version_map ath12k_wifi6_hw_ver_map[] = {
	[ATH12K_HW_QCN9074_HW10] = {
		.hal_params = &ath12k_wifi6_hw_hal_params_qcn9074,
		.hw_regs = &qcn9074_regs,
	},
};
...
static
void ath12k_wifi6_hal_srng_update_hp_tp_addr(struct ath12k_base *ab,
					     int shadow_cfg_idx,
					     ...
					     int ring_num)
{
	...
	ring_id = ath12k_wifi6_hal_srng_get_ring_id(hal, ring_type, ring_num,
						    0);
	if (ring_id < 0)
		return;
					       ...
					       int ring_num)
{
	...
	ath12k_wifi6_hal_srng_update_hp_tp_addr(ab, shadow_cfg_idx, ring_type,
						ring_num);
...
void ath12k_wifi6_hal_set_link_desc_addr(struct hal_wbm_link_desc *desc,
					 ...
					 u8 rbm)
{
	...
	desc->buf_addr_info.info1 =
		le32_encode_bits(((u64)paddr >> HAL_ADDR_MSB_REG_SHIFT),
				WIFI6_BUFFER_ADDR_INFO1_ADDR) |
		le32_encode_bits(rbm, WIFI6_BUFFER_ADDR_INFO1_RET_BUF_MGR) |
		le32_encode_bits(cookie, WIFI6_BUFFER_ADDR_INFO1_SW_COOKIE);
}

u32 ath12k_wifi6_hal_ce_dst_status_get_length(struct hal_ce_srng_dst_status_desc *desc)
{
	u32 len;
...
u32 ath12k_wifi6_hal_tx_read_bank_register(struct ath12k_base *ab, u8 bank_id)
{
	u32 reg_val;
							  ...
							  struct hal_srng *srng)
{
	...
	curr_desc = srng->ring_base_vaddr + srng->u.src_ring.hp;
					       ...
					       struct hal_srng *srng)
{
	...
	next_hp = (srng->u.src_ring.hp + srng->entry_size) % srng->ring_size;
	next_next_hp = (srng->u.src_ring.hp + (srng->entry_size * 2)) %
				srng->ring_size;

	if (next_hp == srng->u.src_ring.cached_tp ||
	    next_next_hp == srng->u.src_ring.cached_tp)
		return NULL;

	desc = srng->ring_base_vaddr + next_next_hp;
