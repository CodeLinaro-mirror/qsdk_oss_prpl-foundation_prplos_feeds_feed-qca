...
smp_affinity_settings() {
	...
	if [ "$enable_smp_affinity" -eq 1 ] && [ -f "/lib/smp_affinity_settings.sh" ]; then
		...
		echo "enabling smp_affinity settings"
	fi
}
